...
function loadComponent(sectionId, filePath, callback) {
  ...
  fetch(filePath)
    ...
    .then((data) => {
      ...
      // 🚀 Auto-start carousel when home.html is loaded
      ...
      setTimeout(() => {
        ...
        if (sectionId === "home") {
          ...
          if (homeCarousel) {
            ...
            console.log("✅ Home carousel started automatically");
          }
        }
...
function handleCareersFormSubmit(form) {
  ...
  const mailtoLink = `mailto:vishnu@gradarchitects.com?subject=${encodeURIComponent(
    subject
  ...
  submitBtn.innerHTML =
    '<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> Opening Email...';
  submitBtn.disabled = true;
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      ...
      submitBtn.disabled = false;
      showAlert(
        "Thank you for your application! Please send the pre-filled email to complete your application.",
        "success"
      );
    }, 2000);
  }, 1000);
...
async function saveToGoogleSheets(formData) {
  const scriptURL =
    "https://script.google.com/macros/s/AKfycbx0NylMw9o6v3LhJbrgc9yy-63GtAENElbt3f75FyNgy1R-F94i0Ujq-WD_7ZBJkno/exec";
...
function initializeMobileFooterNavigation() {
  ...
  mobileNavLinks.forEach((link) => {
    ...
    newLink.addEventListener("click", function (e) {
      ...
      if (
        href.startsWith("/components/project/") ||
...
function initMobileNavigation() {
  ...
  document
    .querySelectorAll('.mobile-menu .nav-link[href^="#"]')
    .forEach((link) => {
      ...
      if (href.includes("#")) {
        ...
        if (isOnProjectPage) {
          ...
          const mainNavLink = document.querySelector(
            `.navbar-nav .nav-link[href="#${section}"]`
          );
          if (mainNavLink) {
            handleNavigationClick(section, mainNavLink);
          } else {
            console.error(
              "❌ Could not find navbar link for section:",
              section
            );
          }
        }
